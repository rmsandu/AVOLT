...
import utils.mev_miv_scatter as mev_miv_scatter
import utils.subplots_pav_eav_scatter as subplots_pav_eav
...
def plot_scatter_pav_eav(df_radiomics,
    ...
    if ratio_flag is False:
        ...
        df['R(EAV:PAV)'] = df['EAV'] / df['PAV']
        df.dropna(inplace=True)
        slope, intercept, r_square, p_value, std_err = stats.linregress(df['major_axis_length_ablation'],
                                                                        df['Energy (kJ)'])
...
def connected_mev_miv(df_radiomics):
    ...
    df.columns = ['Predicted Ablation', 'Effective Ablation', 'Maximum Inscribed Ellipsoid',
